const GetProduct = () => {
	return (dispatch) => {
		fetch(
			"https://v1.nocodeapi.com/cyphertron89/airtable/ksiStlVqpZuQRUrx?tableName=products",
		)
			.then((res) => res.json())
			.then((data) => {
				dispatch({
...
const GetProductDetails = (id) => {
	return (dispatch) => {
		fetch(
			`https://v1.nocodeapi.com/cyphertron89/airtable/ksiStlVqpZuQRUrx?tableName=products&id=${id}`,
		)
			.then((res) => res.json())
			.then((datas) => {
